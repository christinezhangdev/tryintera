...
// === Methodology page: sticky TOC active link ===
(function() {
  const toc = document.querySelector('[data-methodology-toc]');
  if (!toc) return;

  const tocLinks = Array.from(toc.querySelectorAll('.methodology-toc-link'));
  const sectionLinks = Array.from(toc.querySelectorAll('.methodology-section-link'));
  const links = [...tocLinks, ...sectionLinks];
  ...
  function setActiveLink(targetLink) {
    links.forEach(l => {
      l.classList.remove('methodology-toc-link-active');
      l.classList.remove('methodology-section-link-active');
    });
    if (targetLink.classList.contains('methodology-toc-link')) {
      targetLink.classList.add('methodology-toc-link-active');
    }
    if (targetLink.classList.contains('methodology-section-link')) {
      targetLink.classList.add('methodology-section-link-active');
    }
  }
...
// === Nav mobile menu (hamburger + full-screen overlay) ===
(function() {
...
// Nav scroll shadow (stronger shadow when page scrolled)
(function() {
...
// === Team CTA: stagger fade-in on scroll ===
(function() {
  const el = document.getElementById('team-cta');
  if (!el) return;
...
// === DCT Use Cases accordion ===
(function() {
  const items = document.querySelectorAll('.dct-accordion-item');
  if (!items.length) return;

  items.forEach(item => {
    const toggle = item.querySelector('.dct-accordion-toggle');
    const icon = item.querySelector('.dct-accordion-icon');
    const panel = item.querySelector('.dct-accordion-panel');
    const inner = item.querySelector('.dct-accordion-panel-inner');
    if (!toggle || !panel || !inner) return;

    function setOpen(el, open) {
      const p = el.querySelector('.dct-accordion-panel');
      const i = el.querySelector('.dct-accordion-icon');
      const innerEl = el.querySelector('.dct-accordion-panel-inner');
      if (!p || !innerEl) return;
      if (open) {
        el.classList.add('is-open');
        p.style.maxHeight = innerEl.offsetHeight + 'px';
        if (i) i.textContent = '−';
      } else {
